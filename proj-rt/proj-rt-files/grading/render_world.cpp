...
void Render_World::Render_Pixel(const ivec2& pixel_index)
{
    ...
    ray.endpoint = camera.position; //set ray's position to that of the Camera
...
vec3 Render_World::Cast_Ray(const Ray& ray,int recursion_depth)
{
    ...
    if (closest.object != nullptr) {
      color = closest.object->material_shader->Shade_Surface(ray, ray.Point(closest.dist), closest.object->Normal(ray.Point(closest.dist),1), recursion_depth);
    }
    else {
